...
def state_to_strips(env, id, state):
    strips_state = set([])
    strips_state.add('in_' + str(grid[state[0]][state[1]][2]))
    for flag in env.flags_taken[id]:
        strips_state.add('taken_flag' + flag)
    return strips_state

shaper = STRIPS(strips_plan=strips_plans_individual[0], convert=lambda id,state:state_to_strips(environment,id,state), omega=600/9)

#agent
e_greedy = EGreedy(epsilon=0.1)
grid_size = np.shape(grid)
no_features = Feature(state_length= grid_size[0]*grid_size[1])
trace = Eligibility(lambda_=0.99, actions=actions, shape=(no_features.num_features(), len(actions)))
sarsa = Sarsa(actions=actions, alpha=0.1, gamma=0.4, policy=e_greedy, features=no_features, trace=trace)
learners = [RewardShaping(learner=sarsa, shaper=shaper, id=i) for i in range(nr_of_agents)]
